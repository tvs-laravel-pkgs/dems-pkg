...
class PettyCashManagerVerificationController extends Controller {
	public function listPettyCashVerificationManager() {
		$petty_cash = PettyCashEmployeeDetails::select(
			...
			->where('petty_cash_employee_details.status', 3280)
	...
	public function pettycashManagerVerificationSave(Request $request) {
		try {
			DB::beginTransaction();
			if ($request->approve) {
				$petty_cash_manager_approve = PettyCashEmployeeDetails::where('id', $request->approve)->update(['status' => 3281, 'remarks' => '', 'rejection_id' => NULL]);
				DB::commit();
				return response()->json(['success' => true]);
			} else {
				$petty_cash_manager_reject = PettyCashEmployeeDetails::where('id', $request->reject)->update(['status' => 3282, 'remarks' => $request->remarks,, 'rejection_id' => $request->rejection_id]);
				DB::commit();
				return response()->json(['success' => true]);
			}
