app.component('eyatraExpenseVoucherAdvanceFinancierRepaidList', {
    templateUrl: expense_voucher_advance_financier_repaid_list_template_url,
    controller: function(HelperService, $rootScope, $scope, $http, $location) {
        var self = this;
        self.hasPermission = HelperService.hasPermission;
        $http.get(
            expense_voucher_advance_financier_repaid_filter_url
        ...
        var dataTable = $('#eyatra_expense_voucher_financier_repaid_list_table').DataTable({
            stateSave: true,
            ...
            ordering: false,
            ajax: {
                url: laravel_routes['listExpenseVoucherFinancierRepaidList'],
                type: "GET",
                dataType: "json",
                data: function(d) {
        ...
        setTimeout(function() {
            ...
            var d = document.getElementById('eyatra_expense_voucher_financier_repaid_list_table_filter');
            x.left = x.left + 15;
        ...
        $('#approve').on('click', function() {
            ...
            $http.post(
                expense_voucher_advance_financier_multiple_approval_repaid_url, { approve_ids: approve_ids },
            ).then(function(response) {
                if (!response.data.success) {
                    ...
                    $('#approve').css({ 'display': 'none' });
                    var dataTableFilter = $('#eyatra_expense_voucher_financier_repaid_list_table').dataTable();
                    dataTableFilter.fnFilter();
                    //$location.path('/expense/voucher-advance/financier/repaid/list');
                    //$scope.$apply();
        ...
        $(document.body).on('click', '.approve_claim', function() {
            ...
            $http.post(
                expense_voucher_advance_financier_single_approval_repaid_url, { id: id },
            ).then(function(response) {
                if (!response.data.success) {
                    var errors = '';
                    for (var i in res.errors) {
...
app.component('eyatraExpenseVoucherAdvanceFinancierRepaidView', {
    templateUrl: expense_voucher_advance_financier_repaid_view_template_url,
    controller: function($http, $location, $routeParams, HelperService, $rootScope) {
        var self = this;
        self.hasPermission = HelperService.hasPermission;
        $http.get(
            expense_voucher_advance_financier_repaid_view_data_url + '/' + $routeParams.id
        ).then(function(response) {
