...
class LocalTravelModeController extends Controller {

	public function listEYatraLocalTravelMode(Request $r) {
		...
		return Datatables::of($entities)
			->addColumn('action', function ($entity) {
				...
				$action .= '<a style="' . $edit_class . '" href="#!/local-travel-mode/edit/' . $entity->id . '">
	...
	public function eyatraLocalTravelModeFormData($travel_mode_id = NULL) {
		...
		if (!$travel_mode_id) {
			...
			$entity->category_id = DB::table('local_travel_mode_category_type')->where('travel_mode_id', $entity->id)->pluck('category_id')->first();
			if (!$entity) {
		...
		$type_list = collect(Config::amountCategoryList())->prepend(['id' => '', 'name' => 'Select Category']);
	...
	public function saveEYatraLocalTravelMode(Request $request) {
		// dd($request->all());
		...
		try {
			$error_messages = [
				...
				'hsn_code'=>'HSN Code has already been taken',
			];

			$validator = Validator::make($request->all(), [
				'name' => 'required|unique:entities,name,' . $request->id . ',id,company_id,' . Auth::user()->company_id . ',entity_type_id,503',
			], $error_messages);
			...
			$entity->local_travel_mode_categories()->sync($request->category_id);
			DB::commit();
			if (empty($request->id)) {
	...
	public function deleteEYatraLocalTravelMode($travel_mode_id) {
