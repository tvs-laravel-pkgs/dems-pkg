...
class ExpenseAdvanceFinancierRepaidController extends Controller {
	// public function listExpenseVoucherFinancierRepaidList(Request $r) {
	...
	// 			return '
	// 			<a href="#!/expense/voucher-advance/financier/repaid/view/' . $expense_voucher_requests->id . '">
	...
	public function listExpenseVoucherFinancierRepaidList(Request $r) {
		...
		$expense_voucher_requests = ExpenseVoucherAdvanceRequest::select(
			...
			// ->whereNotNull('expense_voucher_advance_requests.balance_amount')
			->whereNotNull('expense_voucher_advance_request_claims.balance_amount')
			->where('users.user_type_id', 3121)
	...
	//OLD 19th JULY 2023
	// public function expenseVoucherFinancierSingleRepaidApprove(Request $request) {
	// 	//dd($request->all());
	// 	DB::beginTransaction();
	// 	try {
	// 		if ($request->id) {
	...
	public function expenseVoucherFinancierSingleRepaidApprove(Request $request) {
		...
		try {
			...
			$reimbursement_amount = $outlet ? $outlet->reimbursement_amount > 0 ? $outlet->reimbursement_amount : 0 : 0;
			$balance_amount = $reimbursement_amount + $expense_voucher_advance_request_claim->balance_amount;
			...
			$reimbursementtranscation->transaction_date = Carbon::now();
			$reimbursementtranscation->petty_cash_id = $request->id;
			$reimbursementtranscation->amount = $expense_voucher_advance_request_claim->balance_amount;
			$reimbursementtranscation->balance_amount = $balance_amount;
			$reimbursementtranscation->save();
	...
	//OLD 19 TH JULY 2023
	// public function expenseVoucherFinancierMultipleRepaidApprove(Request $request) {
	// 	DB::beginTransaction();
	// 	try {
