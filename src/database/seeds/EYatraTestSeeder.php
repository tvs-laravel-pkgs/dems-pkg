...
class EYatraTestSeeder extends Seeder {
	/**
	...
	public function run() {
		...
		$com_data['data']['code'] = 'TVS00' . $company_id;
		$com_data['data']['name'] = 'TVS';
		//$com_data['data']['name'] = 'Company ' . $company_id;
		$com_data['data']['address'] = $faker->streetAddress;
		$com_data['data']['cin_number'] = 'CIN123456789';
		$com_data['data']['gst_number'] = 'GST123456789';
		$com_data['data']['customer_care_email'] = 'support@tvs.in';
		$com_data['data']['customer_care_phone'] = '0422' . rand(10000, 99999);
		$com_data['data']['reference_code'] = 'TVSR000' . $company_id;
		...
		$admin->mobile_number = rand(1000000000, 9999999999);
		$admin->name = 'Admin';
		...
		foreach ($lobs as $key => $lob_sbu) {
			//dd($key, $lob_sbu['name']);
			$lob = Lob::firstOrNew([
				'company_id' => $company->id,
				'name' => $lob_sbu['name'],
			]);
			$lob->created_by = $admin->id;
			$lob->save();
			//dd($lob_sbu['sbus']);
			foreach ($lob_sbu['sbus'] as $key => $sbu) {
				//dd($key, $sbu);
				$sbu = Sbu::firstOrNew([
					'lob_id' => $lob->id,
					'name' => $sbu,
				]);
				$sbu->created_by = $admin->id;
				$sbu->save();
			}
		}
...
/** doubt**/
		$sample_entities = [
			...
			518 => [
				...
				'Premimum Tatkal',
				'AC TAKAL',
				'AC GENERAL -(3RD AC & 2ND AC) ',
				'EC',
				'1ST AC',
				'GENERAL',
				'TAKAL',
				'SLEEPER CLASS NON-AC - GENERAL',
				'SLEEPER CLAS NON-AC – TAKAL',
				'AC CHAIR CAR – GENERAL',
				'AC CHAIR CAR -TAKAL',
				'TAKAL & GENERAL - NON-AC SEATING',
				'BUS AC',
				'BUS NON-AC',
				'FLIGHT',
			],
		...
		for ($i = 1; $i <= $number_of_items; $i++) {
			...
			$code = 'EMP' . $i;
			$cashier = Employee::create2($company, $outlet, $admin, $faker);
			$this->command->info('------------------');
			$this->command->info('Cashier Created : ' . $cashier->code);

			$user_type_id = 3121;
			$cashier_user = Employee::createUser($company, $user_type_id, $cashier, $faker, rand(1000000000, 9999999999), $roles = 504);
			...
			$cashier = Employee::create2($company, $outlet, $admin, $faker);
			$this->command->info('------------------');
			$this->command->info('Financier Created : ' . $cashier->code);

			$user_type_id = 3121;
			$cashier_user = Employee::createUser($company, $user_type_id, $cashier, $faker, rand(1000000000, 9999999999), $roles = 505);

			//Sr.MANAGER
			$code = $outlet->code . '/sm' . $i;
			$sr_manager = Employee::create2($company, $outlet, $admin, $faker);
			$this->command->info('------------------');
			$this->command->info('Sr.Manager Created : ' . $sr_manager->code);

			//USER ACCOUNT
			$user_type_id = 3121;
			$user = Employee::createUser($company, $user_type_id, $sr_manager, $faker, rand(1000000000, 9999999999), $roles = 502);
			...
			for ($j = 1; $j <= $number_of_items; $j++) {
				...
				//MANAGERS
				$code = 'MANAGER001';
				$manager = Employee::create2($company, $outlet, $admin, $faker, $sr_manager->id);
				$this->command->info('------------------');
				$this->command->info('Manager Created : ' . $manager->code);

				//USER ACCOUNT
				$user_type_id = 3121;
				$user = Employee::createUser($company, $user_type_id, $manager, $faker, rand(1000000000, 9999999999), $roles = 502);

				//EMPLOYEES - REGULAR
				for ($k = 1; $k <= $number_of_items; $k++) {
					$code = 'EMPR001';
					$employee = Employee::create2($company, $outlet, $admin, $faker, $manager->id);
					$this->command->info('------------------');
					$this->command->info('Employee Created : ' . $employee->code);

					$user_type_id = 3121;
					$user = Employee::createUser($company, $user_type_id, $employee, $faker, rand(1000000000, 9999999999), $roles = 501);
				}
			}
		}
		...
		for ($i = 1; $i <= $number_of_items; $i++) {
			...
			$user_type_id = 3122;
			$user = Employee::createUser($company, $user_type_id, $agent, $faker, rand(1000000000, 9999999999), $roles = 503);
